...
## AI-powered risk analyzer (JavaScript)

Added `analyzer/analyzeContract.js` with:
- `analyzeContract(codeOrAbi)`
- `RISK_ANALYZER_PROMPT_TEMPLATE` for LLM-enhanced analysis
...
// Optional: inject contract text into prompt for LLM usage
const prompt = RISK_ANALYZER_PROMPT_TEMPLATE.replace("{{CONTRACT_INPUT}}", source);
console.log(prompt);
```
...
```json
{
  "score": 85,
  "level": "HIGH",
  "summary": "Risk signals: owner privileges detected, mint capability detected, blacklist capability detected, pausable transfer controls detected."
}
```
